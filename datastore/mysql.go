...
const CREATE_CUSTOMERS_TABLE = `CREATE TABLE IF NOT EXISTS CUSTOMERS(
...
func New() (DB, error) {
	...
	var (
		customername = os.Getenv("SQL_CUSTOMER")
	...
	db, err := otelsql.Open("mysql", datasourceName(customername, password, host, ""), otelsql.WithAttributes(
	...
	if _, err := db.Exec("CREATE DATABASE IF NOT EXISTS " + dbName); err != nil {
		return nil, fmt.Errorf("atatus db create error: %w", err)
	}

	// close the exising connection. db.Close() is idempotent. Hence, it is safe to close the db here.
	db.Close()

	db, err = otelsql.Open("mysql", datasourceName(customername, password, host, dbName))
	if err != nil {
		return nil, fmt.Errorf("open atatus db error: %w", err)
	}
...
func datasourceName(customername, password, host, dbName string) string {
	return fmt.Sprintf("%s:%s@tcp(%s)/%s", customername, password, host, dbName)
}

func createTables(db *sql.DB) error {
	if _, err := db.Exec(CREATE_CUSTOMERS_TABLE); err != nil {
		return fmt.Errorf("create customer table error: %w", err)
	}
